# 🐍 Python Practice Problems

Beginner Python practice – basics, data types and data structures.

---

## 📁 Notebooks

| File | Topics Covered |
|------|---------------|
| `Python_Practice_Problems.ipynb` | Print, Comments, Variables & Data Types, Strings, Operations, User Input,  
Control Flow, Loops, Lists, Functions |
| `Python_Practice_Problems_2.ipynb` | Data Structures – Tuples, Dictionaries |

---

## 📚 Topics

- ✅ Print & Comments
- ✅ Variables & Data Types
- ✅ Strings & Operations
- ✅ User Input
- ✅ Control Flow (if/else)
- ✅ Loops (for, while)
- ✅ Lists
- ✅ Functions
- ✅ Tuples
- ✅ Dictionaries

---

## 🛠️ How to Run

1. Clone the Repository :
   ```bash
   git clone https://github.com/your-username/python-practice-problems.git
   ```
2. Open with Jupyter Notebook or [Google Colab](https://colab.research.google.com/)

---

## 🤝 Contributing

Contributions are welcome! If you'd like to improve or add new practice problems :

1. Fork the repository
2. Create a new branch :
   ```bash
   git checkout -b feature/new-problems
   ```
3. Commit your changes :
   ```bash
   git commit -m "Add new practice problems"
   ```
4. Push to the branch :
   ```bash
   git push origin feature/new-problems
   ```
5. Open a **Pull Request**

